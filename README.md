...
4. Update the `app-config.yaml`:
    ...
    * Add this root property `dora` to the file and then add the following under it:
		* Required:
			...
			* `daysToFetch`: This will be used to talk to the API and tell it how many days in the past to fetch.  We recommend at least 365 days, but you can do more or less.

		* Optional:
			* `showTrendGraph`: Setting this to true will replace the Metric Board with a Trend Graph
			* `showIndividualMetricTrends`: Setting this to true to cause individual metric trends to show on the Trend Graph
			* `teams`: This is an array of strings (team names) containing a list of teams you want to have available on the team view page, only use this if you don't not have an API function to gathering your team names
